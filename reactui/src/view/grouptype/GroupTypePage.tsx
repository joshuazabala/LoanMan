...
export default class GroupTypePage extends React.Component<any, IState> {
            ...
            <Container fluid={true}>
                <Grid>
                                ...
                                <TableBody>
                                    {
                                        this.state.contents.map((item, index) => {
                                            return (
                                                <TableRow key={index} data-grouptypeid={item.id} onClick={this.onRowClick} active={this.state.selectedId === item.id}>
                    ...
                    <Modal open={this.state.deleteModalVisible} size="small">
                        ...
                        <ModalContent>Delete selected group type?</ModalContent>
                        <ModalActions>
                            <Button
                                negative={true}
                                content="Delete"
                                onClick={this.deleteGroupType}
                                disabled={this.state.loading}
                            />
                            <Button
                                content="Cancel"
    ...
    private onRowClick = (event: any) => {
        const groupTypeId = event.currentTarget.getAttribute("data-grouptypeid") as number;
    ...
    private showGroupTypeForm = (id: number) => {
        ...
        fetchPost<{ id: number }, GroupType>("/grouptype/findById", requestParam)
            .then(item => {
                this.setState(
                    { formVisible: true },
                    () => {
    ...
    private onFormSaved = () => {
        this.formRef.current!.setState(
            ...
            () => {
                ...
                const requestParam = new RequestContainer<GroupType>();
                requestParam.content = content;

                fetchPost<RequestContainer<GroupType>, ResponseContainer<GroupType>>("/grouptype/save", requestParam)
                    .then(response => {
                        this.formRef.current!.setState(
                            { loading: false },
                            () => {
    ...
    private deleteGroupType = () => {
        ...
        this.setState(
            { loading: true },
            () => {
                fetchPost<RequestContainer<number>, ResponseContainer<boolean>>("/grouptype/delete", requestParam)
                    .then(response => {
                        if (response.status === EnumResponseStatus.SUCCESSFUL) {
