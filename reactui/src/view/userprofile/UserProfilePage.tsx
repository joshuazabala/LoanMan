...
interface IState extends IPageState<UserProfile> {
...
export default class UserProfilePage extends React.Component {
    ...
    public state: IState = {
        contents: new Array<UserProfile>(),
    ...
    private formRef = React.createRef<UserProfileForm>();
    ...
    private onRowClick = (event: any) => {
        const userProfileId = event.currentTarget.getAttribute("data-id") as number;
        this.setState({ selectedId: +userProfileId });
    }
    ...
    private showForm = (id: number) => {
        fetchGet<UserProfile>("/userprofile/findById/" + id)
            .then(item => {
                this.setState(
                    { formVisible: true },
                    () => {
    ...
    private onFormSaved = () => {
        this.formRef.current!.setState(
            ...
            () => {
                ...
                const url = "/userprofile/" + (content.id === 0 ? "create" : "edit");
                fetchPost<UserProfile, ResponseContainer<UserProfile>>(url, content)
                    .then(response => {
                        this.formRef.current!.setState(
                            { loading: false },
                            () => {
    ...
    private deleteOrRestore = (mode: EnumCommonAction.DELETE | EnumCommonAction.RESTORE, id: number) => {
        const url = "/userprofile/" + (mode === EnumCommonAction.RESTORE ? "restore" : "delete") + "/" + id;
        this.setState(
            { loading: true },
            () => {
